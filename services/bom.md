...
`nvpair-sensors` (Windows-only) likewise links only `go-winio` (its named pipe) and `golang.org/x/sys` (service control, registry, the PawnIO library calls). `nvpair-node-info` now links `go-winio` on Windows to read that pipe. Beyond Go modules, `nvpair-sensors` embeds a signed PawnIO module blob (`IntelMSR.bin`, LGPL-2.1-or-later, handed to the driver as data and not linked); see `nvpair-sensors/modules/NOTICE.md`.
...
| `golang.org/x/sys` | v0.47.0 | nvpair-node-info, nvpair-cluster-manager, nvpair-engine-manager, nvpair-sensors | BSD-3-Clause | [LICENSE](https://cs.opensource.google/go/x/sys/+/master:LICENSE) |
